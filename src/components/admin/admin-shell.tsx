 ...
 */
export function AdminShell({ email, signOutQuiet, signOutButton, children }: Props) {
  return (
    <div className="flex min-h-screen">
      <aside className="sticky top-0 hidden h-screen w-60 shrink-0 flex-col bg-sidebar px-3.5 py-[18px] text-sidebar-foreground lg:flex">
        <span className="px-2 pb-[18px] font-heading text-lg font-semibold tracking-tight text-sidebar-accent-foreground">
          echoandaura
        </span>
        <AdminNav />
        ...
        <header className="flex h-14 items-center justify-between gap-3 border-b border-border bg-card px-3 lg:h-16 lg:px-6">
          <div className="flex items-center gap-3">
            <div className="lg:hidden">
              <MobileNav email={email} signOut={signOutQuiet} />
            </div>
