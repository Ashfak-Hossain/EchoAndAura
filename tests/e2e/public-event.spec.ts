...
const SITE = (process.env.SITE_URL ?? process.env.BETTER_AUTH_URL ?? 'http://localhost:3000').replace(/\/+$/, '');
...
async function openTab(page: Page, name: 'Details' | 'Cover image' | 'Ticket types' | 'Publish') {
  await page.getByRole('navigation', { name: /event sections/i }).getByRole('link', { name }).click();
}
...
async function createPublishableEvent(
  ...
  for (const [name, price, qty] of [
    ...
    await page.getByRole('link', { name: /add ticket type/i }).first().click();
...
test.describe('public event page (A2)', () => {
  test('PHASE 2 EXIT: a published event renders with Open Graph tags for Facebook', async ({
    ...
    await expect(page.getByRole('link', { name: /share on facebook|^facebook$/i }).first()).toHaveAttribute(
      'href',
      /facebook\.com\/sharer/,
    );
    ...
    expect(await meta('og:image')).toMatch(/^http:\/\/localhost:9000\/.+\/events\/.+\/cover-.+\.png$/);
  ...
  test('a past (archived) event keeps its page without a CTA', async ({ page }) => {
    ...
    await page.getByRole('alertdialog').getByRole('button', { name: /archive event/i }).click();
